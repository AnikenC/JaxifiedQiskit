...
class DynamicsBackendEstimator(BaseEstimator[PrimitiveJob[EstimatorResult]]):
    ...
    def _transpile(self):
        ...
        for common_circuit, diff_circuits in self.preprocessed_circuits:
            ...
            passmanager = _passmanager_for_measurement_circuits(
                perm_pattern, self.backend
            )
            ...
            for diff_circuit in diff_circuits:
                ...
                transpiled_circuit_copy.compose(
                    diff_circuit, clbits=clbits, inplace=True
                )
        ...
        for group in self._grouping:
            ...
            if self._abelian_grouping:
                for obs in observable.group_commuting(qubit_wise=True):
                    ...
                    meas_circuit, indices = self._measurement_circuit(
    ...
    def _postprocessing(
        ...
        metadata = [
            {"variance": var, "shots": shots}
            for var, shots in zip(var_list, shots_list)
        ]
...
def _passmanager_for_measurement_circuits(layout, backend) -> PassManager:
    ...
    if isinstance(backend, BackendV2) and isinstance(backend.coupling_map, CouplingMap):
        ...
        isinstance(backend, BackendV1)
        and backend.configuration().coupling_map is not None
    ):
